...
import { Text, View, StatusBar, TextInput, Image, TouchableOpacity, TouchableNativeFeedback, ScrollView, Modal, ToastAndroid, Alert, Linking, Switch, RefreshControl } from 'react-native'
...
class Dashboard extends Component {
  ...
  getReject() {
    this.setState({ loading: true })
    fetch("https://sampahbank.herokuapp.com/api/pickup/rejection", {
      method: "GET",
      headers: {
  ...
  getAccept() {
    this.setState({ loading: true })
    fetch("https://sampahbank.herokuapp.com/api/pickup/done", {
      method: "GET",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${this.props.userData.userReducer.user}`
  ...
  getJemput() {
    this.setState({ loading: true })
    fetch("https://sampahbank.herokuapp.com/api/pickup/list", {
      method: "GET",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${this.props.userData.userReducer.user}`
        ...
        <View style={style.contTab1}>
          <TouchableOpacity onPress={() => this.setState({ screen: 1 })}>
            ...
            <Text style={
              this.state.screen == 1
                ...
                : [style.textTab, { fontSize: 10 }]}>Permintaan</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => this.setState({ screen: 2 })}>
            <Image
              source={require("../../assets/userinterface.png")}
              style={
                this.state.screen == 2
                  ? style.onIcon2
                  : style.icon2
            ...
            <Text style={
              this.state.screen == 2
                ? [style.textTab, { fontSize: 12, fontWeight: "700" }]
                : [style.textTab, { fontSize: 10, marginTop: 3 }]}>Di Terima</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => this.setState({ screen: 3 })}>
            ...
            <Text style={
              this.state.screen == 3
                ? [style.textTab, { fontSize: 12, fontWeight: "700" }]
                : [style.textTab, { fontSize: 10, marginTop: 3 }]}>Di Tolak</Text>
          </TouchableOpacity>
        </View>
        {this.state.loading
          ...
          : <ScrollView
            ...
            {this.state.screen == 1
              ? this.state.jemput == ""
                ...
                : (this.state.jemput.map((item, index) => (
                  <TouchableNativeFeedback key={index} onPress={() => this.props.navigation.navigate("Angkut", { value: item })}>
                    <View style={style.list1}>
                      <View style={{ marginHorizontal: 15 }}>
                        <Text style={[style.text, { fontSize: 10 }]}>ID Permintaan Angkut: #{item.id}</Text>
                        <Text numberOfLines={1} style={[style.barang, { marginVertical: 5 }]}>Nama : {item.user.name}</Text>
                      </View>
                      <View style={{ marginHorizontal: 15 }}>
              ...
              : this.state.screen == 2
                ? this.state.accept == ""
                  ...
                  : (this.state.accept.map((item, index) => (
                    <TouchableNativeFeedback key={index} onPress={() => this.props.navigation.navigate("Setor", { value: item })}>
                      <View style={style.list1}>
                        <View style={{ marginHorizontal: 15 }}>
                          <Text style={[style.text, { fontSize: 10 }]}>ID Permintaan Angkut: #{item.id}</Text>
                          <Text numberOfLines={1} style={[style.barang, { marginVertical: 5 }]}>Nama : {item.user.name}</Text>
                ...
                : this.state.reject == ""
                  ? <Text style={{ textAlign: "center" }}>Data Kosong</Text>
                  : (this.state.reject.map((item, index) => (
                    <TouchableNativeFeedback key={index} onPress={() => this.props.navigation.navigate("Angkut", { value: item })}>
                      <View style={style.list1}>
                        <View style={{ marginHorizontal: 15 }}>
                          <Text style={[style.text, { fontSize: 10 }]}>ID Permintaan Angkut: #{item.id}</Text>
